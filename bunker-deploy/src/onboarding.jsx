...
const DARK = {
  "--bg":"#0e0f11","--bg-surface":"#16171b","--bg-elevated":"#1c1d22",
...
const iStyle = { width:"100%", padding:"10px 14px", borderRadius:8, border:"1px solid var(--border)", background:"var(--bg-elevated)", color:"var(--text)", fontSize:13, fontFamily:"'DM Sans',sans-serif", outline:"none", boxSizing:"border-box", transition:"border-color 0.2s" };
      ...
      {STEPS.map((s, i) => (
        <div key={s} style={{ display:"contents" }}>
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:6 }}>
            <div style={{ width:28, height:28, borderRadius:99, display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:700, background: i<=step ? "var(--amber)" : "var(--bg-elevated)", border: i<=step ? "none" : "1px solid var(--border)", color: i<=step ? "#0e0f11" : "var(--muted)", transition:"all 0.3s" }}>
              {i < step ? "✓" : i+1}
            </div>
...
function NavBtns({ onBack, onNext, nextLabel="Continue →", nextDisabled=false, loading=false }) {
  return (
    <div style={{ display:"flex", justifyContent:"space-between", marginTop:28 }}>
      {onBack
        ? <button onClick={onBack} style={{ padding:"10px 20px", borderRadius:8, border:"1px solid var(--border)", background:"transparent", color:"var(--text-secondary)", fontSize:13, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>← Back</button>
        : <div />}
    ...
    <div style={{ padding:"32px 40px" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
        <div>
          ...
          <input style={iStyle} placeholder="e.g. Cask & Stream, Trail Notes…" value={data.name} onChange={e => onChange({...data,name:e.target.value})} autoFocus onFocus={e=>e.target.style.borderColor="var(--amber)"} onBlur={e=>e.target.style.borderColor="var(--border)"} />
        </div>
        <div>
          ...
          <input style={iStyle} placeholder="caskandstream.com" value={data.url} onChange={e => onChange({...data,url:e.target.value})} onFocus={e=>e.target.style.borderColor="var(--amber)"} onBlur={e=>e.target.style.borderColor="var(--border)"} />
        </div>
        <div>
        ...
        <div>
          ...
          <textarea rows={2} style={{ ...iStyle, resize:"none" }} placeholder="What's your blog about?" value={data.desc} onChange={e => onChange({...data,desc:e.target.value})} onFocus={e=>e.target.style.borderColor="var(--amber)"} onBlur={e=>e.target.style.borderColor="var(--border)"} />
        </div>
      </div>
...
function StepPlatform({ data, onChange, onNext, onBack }) {
  ...
  return (
    <div style={{ padding:"32px 40px" }}>
      ...
      {data.platform==="wix" && (
        data.connected
          ? <div style={{ padding:16, borderRadius:10, border:"1px solid rgba(92,186,108,0.4)", background:"rgba(92,186,108,0.08)", display:"flex", alignItems:"center", gap:12 }}>
              <span style={{ width:10, height:10, borderRadius:99, background:"#5cba6c", display:"inline-block", boxShadow:"0 0 8px rgba(92,186,108,0.5)" }} />
              <span style={{ fontWeight:600, fontSize:13 }}>Wix Blog Connected!</span>
            </div>
          : <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              <div>
                ...
                <input type="password" style={iStyle} placeholder="Paste your Wix Blog API key…" value={data.apiKey} onChange={e => onChange({...data,apiKey:e.target.value})} onFocus={e=>e.target.style.borderColor="var(--amber)"} onBlur={e=>e.target.style.borderColor="var(--border)"} />
              </div>
              <div style={{ fontSize:12, color:"var(--text-secondary)", padding:"10px 12px", borderRadius:8, background:"var(--bg-elevated)", border:"1px solid var(--border)", lineHeight:1.6 }}>
          ...
          <button key={plan.id} onClick={() => onChange({...data,plan:plan.id})} style={{ padding:"16px 20px", borderRadius:12, cursor:"pointer", border: data.plan===plan.id ? `1px solid ${plan.color}` : "1px solid var(--border)", background: data.plan===plan.id ? plan.color+"12" : "var(--bg-elevated)", textAlign:"left", fontFamily:"'DM Sans',sans-serif", display:"flex", alignItems:"center", gap:16 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:"4px 16px" }}>
                {plan.features.map(f => <span key={f} style={{ fontSize:11, color:"var(--text-secondary)" }}>· {f}</span>)}
              </div>
            </div>
          </button>
        ))}
  ...
  return (
    <div style={{ padding:40, textAlign:"center" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10, maxWidth:400, margin:"0 auto 32px", textAlign:"left" }}>
        {[
          { icon:"▤", label:"Workspace", value: wsData.name||"My Blog", color:null, sub: blogType ? `${blogType.icon} ${blogType.label}` : null },
          ...
          <div key={row.label} style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 16px", borderRadius:10, background:"var(--bg-elevated)", border:"1px solid var(--border)" }}>
            ...
            <div>
              <span style={{ fontSize:13, fontWeight:600, color: row.color||"var(--text)" }}>{row.value}</span>
              {row.sub && <span style={{ fontSize:11, color:"var(--text-secondary)", marginLeft:8 }}>{row.sub}</span>}
            </div>
          </div>
        ))}
      </div>
